...
export default function LabelAdmin() {
  ...
  useEffect(() => { void loadProjects(); /* eslint-disable-next-line */ }, []);
  ...
  async function createFirstProject() {
    ...
    if (!r.ok) {
      ...
      alert(t?.error ?? "Failed to create project");
      return;
    }
    await loadProjects();
  }
  ...
  useEffect(() => { if (projectId) void loadLabels(projectId); /* eslint-disable-next-line */ }, [projectId]);
  ...
  async function createLabel() {
    ...
    if (!r.ok) {
      alert((await r.json().catch(() => ({})))?.error ?? "Create failed");
      return;
    }
  ...
  if (!projects.length) {
    return (
      <div className="space-y-4">
        <p className="text-slate-700">You don’t have any projects yet.</p>
        <button
          type="button"
